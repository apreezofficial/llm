            ...
            <section style={{ padding: '160px 0 100px', background: '#046B63', color: 'white', position: 'relative', overflow: 'hidden' }}>
                ...
                <div className="container animate-slide-up" style={{ position: 'relative', zIndex: 1 }}>
                    <div style={{ textAlign: 'center', maxWidth: '860px', margin: '0 auto' }}>
                        <div style={{ background: '#FF7D00', color: 'white', padding: '8px 20px', borderRadius: '30px', fontSize: '11px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '2px', display: 'inline-block', marginBottom: '32px', fontFamily: 'var(--font-orbitron)', boxShadow: '0 4px 12px rgba(255,125,0,0.3)' }}>The Full Suite</div>
                        <h1 style={{ fontSize: 'clamp(40px, 6vw, 72px)', fontWeight: 900, fontFamily: 'var(--font-orbitron)', marginBottom: '28px', lineHeight: 1.1 }}>
                            One product for your<br /> <span style={{ color: '#FF7D00' }}>entire institution.</span>
                        </h1>
                        <p className="animate-fade delay-2" style={{ fontSize: '20px', opacity: 0.8, maxWidth: '700px', margin: '0 auto 48px', lineHeight: 1.7 }}>
            ...
            <section style={{ padding: '120px 0', background: '#F8FAFC' }}>
                <div className="container">
                    <div className="text-center animate-slide-up" style={{ marginBottom: '80px' }}>
                        <h2 style={{ fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 900, fontFamily: 'var(--font-orbitron)', color: 'var(--secondary)' }}>
                            Everything included. <span style={{ color: '#FF7D00' }}>No extras.</span>
                        </h2>
                    </div>
                    <div className="grid grid-3" style={{ gap: '28px' }}>
                        {highlights.map((h, i) => (
                            <div key={i} className={`card animate-rotate-in delay-${(i % 3) + 1}`} style={{ padding: '40px 32px', background: 'white', border: '1px solid var(--border)', borderRadius: '24px', boxShadow: '0 10px 30px rgba(0,0,0,0.03)', transition: 'all 0.3s ease' }}>
                                <div style={{ color: '#FF7D00', marginBottom: '24px' }}>{h.icon}</div>
                                <h3 style={{ fontSize: '20px', fontWeight: 800, marginBottom: '12px', fontFamily: 'var(--font-orbitron)' }}>{h.title}</h3>
